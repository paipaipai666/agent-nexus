...
from agentnexus.tools.registry import RiskLevel, ToolRegistry
...
class TestRegisterCount:
    ...
    def test_registers_correct_tool_names(self):
        ...
        names = executor.list_tools()
        expected = {
            "memory_search", "memory_save", "memory_project_status", "history_search", "grep_search", "web_search",
            ...
            "browser_list_pages", "browser_dismiss_popup", "browser_switch_page",
            "computer_click", "computer_key", "computer_launch",
...
class TestRiskLevels:
    ...
    def test_medium_risk_tool(self):
        executor = ToolRegistry()
        register_all_tools(executor)
        meta = executor._tools["file_write"][0]
...
class TestRateLimits:
    ...
    def test_specific_rates(self):
        executor = ToolRegistry()
        register_all_tools(executor)
        meta = executor._tools["file_read"][0]
...
class TestTimeoutAndAgents:
    def test_python_execute_config(self):
        executor = ToolRegistry()
        register_all_tools(executor)
        meta = executor._tools["python_execute"][0]
    ...
    def test_shell_exec_config(self):
        executor = ToolRegistry()
        register_all_tools(executor)
        meta = executor._tools["shell_exec"][0]
        assert meta.timeout_sec == 60
...
class TestParamSchemas:
    def test_web_search_requires_query(self):
        executor = ToolRegistry()
        register_all_tools(executor)
        meta = executor._tools["web_search"][0]
    ...
    def test_kb_search_exposes_advanced_rag_params(self):
        executor = ToolRegistry()
        register_all_tools(executor)
        meta = executor._tools["kb_search"][0]
...
class TestMCPRegistration:
    def test_registers_mcp_tools_when_manager_present(self):
        ...
        meta = executor._tools["mcp_demo__echo"][0]
        assert meta.risk_level == RiskLevel.MEDIUM
    ...
    def test_include_tools_filters_mcp_tools(self):
        ...
        assert executor.list_tools() == ["mcp_demo__echo"]
