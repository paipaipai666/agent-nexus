...
def test_embed_cold_start(perf_env):
    from agentnexus.rag.embeddings import get_embedding_model, reset_embedding_model
...
def test_embed_single_text(benchmark, perf_env):
    from agentnexus.rag.embeddings import embed_texts, get_embedding_model

    get_embedding_model()
    result = benchmark(embed_texts, ["What is vector search?"])
...
def test_embed_batch_10(benchmark, perf_env):
    ...
    result = benchmark(embed_texts, texts)
    assert isinstance(result, list)
...
def test_embed_batch_scaling_linearity(perf_env):
    ...
    t1 = _time_one(embed_texts, texts_1)
    t10 = _time_one(embed_texts, texts_10)
    t50 = _time_one(embed_texts, texts_50)
...
def test_embed_deterministic(perf_env):
    ...
    get_embedding_model()
    r1 = embed_texts(["deterministic test"])
    r2 = embed_texts(["deterministic test"])
