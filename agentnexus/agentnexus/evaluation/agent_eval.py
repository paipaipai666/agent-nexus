...
class AgentReport:
    ...
    def summary(self) -> str:
        lines = [
            ...
            "",
            f"  答案产出率:  {self.answer_rate:.1%}  {'[PASS]' if self.answer_rate >= 0.85 else '[FAIL]'} (阈值 85%)",
            f"  平均步数:    {self.avg_steps:.1f}",
            f"  工具成功率:  {self.tool_success_rate:.1%}  {'[PASS]' if self.tool_success_rate >= 0.80 else '[FAIL]'} (阈值 80%)",
            f"  截断率:      {self.truncation_rate:.1%}  {'[PASS]' if self.truncation_rate < 0.10 else '[FAIL]'} (阈值 <10%)",
            f"  错误率:      {self.error_rate:.1%}  {'[PASS]' if self.error_rate < 0.10 else '[FAIL]'} (阈值 <10%)",
            "",
            ...
            "",
            f"  延迟 P50: {self.latency_p50_ms:.0f}ms  P95: {self.latency_p95_ms:.0f}ms  P99: {self.latency_p99_ms:.0f}ms",
        ]
        return "\n".join(lines)
