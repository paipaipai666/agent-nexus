    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {failures > 0 && (
            <button onClick={() => handleAction('retry')} disabled={!!actionLoading} className="btn-ghost flex items-center gap-1.5 text-sm" style={{ color: 'var(--amber)' }}>
              <RefreshCw size={14} /> Retry Failed
            </button>
          )}
          <button onClick={() => handleAction('reload')} disabled={!!actionLoading} className="btn-primary flex items-center gap-1.5 text-sm">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        ...
        <div ref={(el) => { if (el) animateEntrance(Array.from(el.children), { stagger: 0.08 }) }} className="grid grid-cols-4 gap-3">
          {[
            ...
            <div key={label} className="p-3.5 rounded-lg" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-card), var(--card-highlight)' }}>
        ...
        <div className="space-y-2">
          {servers.length === 0 ? (
            ...
            <div key={server.name} className="rounded-lg overflow-hidden" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-card), var(--card-highlight)' }}>
                ...
                <div className="px-4 pb-4 space-y-3 animate-slide-up" style={{ borderTop: '1px solid var(--border)' }}>
                  ...
                  {server.tool_names.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">{server.tool_names.map(t => <span key={t} className="text-xs px-1.5 py-0.5 rounded font-mono" style={{ background: 'var(--accent-muted)', color: 'var(--accent)' }}>{t}</span>)}</div>
                    </div>
                  )}
